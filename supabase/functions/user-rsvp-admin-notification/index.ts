...
Deno.serve(async (req) => {
  ...
  const {
    record,
    old_record,
  }: { record?: Tables<"event_invite">; old_record?: Tables<"event_invite"> } =
    await req.json();

  type TriggerType = "created" | "updated" | "deleted";
  let type: TriggerType = "created";
  ...
  const { data: rsvpUser } = await supabase
    ...
    .single();
  let message: { title: string; body: string } = { title: "", body: "" };

  if (type === "created") {
    const rsvp: RsvpStatus = (record!.rsvp ?? "pending") as RsvpStatus;

    const attendingText = "kommer till Purplecon!";
    const tentativeText = "är tentative till Purplecon.";
    const rejectedText = "kommer ej till Purplecon.";
    ...
    message = {
      title: `${rsvpUser!.first_name} ${rsvpUser!.last_name} ${text}`,
      body: `${rsvpUser!.first_name} ${rsvpUser!.last_name} har RSVP:at ${record!.rsvp}`,
    };
  } else if (type === "updated") {
    const rsvp: RsvpStatus = (record!.rsvp ?? "pending") as RsvpStatus;

    message = {
      title: `${rsvpUser!.first_name} ${rsvpUser!.last_name} har ändrat sitt RSVP till ${rsvp}`,
      body: `${rsvpUser!.first_name} ${rsvpUser!.last_name} har ändrat sitt RSVP till ${rsvp}`,
    };
  } else if (type === "deleted") {
    message = {
      title: `${rsvpUser!.first_name} ${rsvpUser!.last_name} har tagit bort sin RSVP`,
      body: `${rsvpUser!.first_name} ${rsvpUser!.last_name} har tagit bort sin RSVP`,
    };
  }
